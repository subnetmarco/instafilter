...
public class GammaFilter extends BaseFilter {
	...
	protected void doFilter(BufferedImage image, OutputStream output,
		...
		float gamma = options.get(FilterOption.GAMMA, Float.class);
		
		BufferedImage outputImage = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_INT_RGB);
		com.jhlabs.image.GammaFilter gammaFilter = new com.jhlabs.image.GammaFilter(gamma);
		gammaFilter.filter(image, outputImage);

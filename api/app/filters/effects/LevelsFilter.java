...
public class LevelsFilter extends BaseFilter {
	...
	protected void doFilter(BufferedImage image, OutputStream output,
		...
		float highLevel = options.get(FilterOption.HIGH_LEVEL, Float.class);
		float lowLevel = options.get(FilterOption.LOW_LEVEL, Float.class);
		float highOutputLevel = options.get(FilterOption.HIGH_OUTPUT, Float.class);
		float lowOutputLevel = options.get(FilterOption.LOW_OUTPUT, Float.class);
		
		BufferedImage outputImage = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_INT_RGB);

		com.jhlabs.image.LevelsFilter levelsFilter = new com.jhlabs.image.LevelsFilter();
		...
		levelsFilter.filter(image, outputImage);
